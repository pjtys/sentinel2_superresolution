#+begin_html
<a href="https://github.com/Evoland-Land-Monitoring-Evolution/sentinel2_superresolution/actions/workflows/ci.yml">
<img alt="Build" src="https://github.com/Evoland-Land-Monitoring-Evolution/sentinel2_superresolution/actions/workflows/ci.yml/badge.svg" />
</a>
<a href="https://github.com/Evoland-Land-Monitoring-Evolution/sentinel2_superresolution/actions/workflows/docker.yml">
<img alt="Docker" src="https://github.com/Evoland-Land-Monitoring-Evolution/sentinel2_superresolution/actions/workflows/docker.yml/badge.svg" />
</a>
<a href="https://opensource.org/licenses/Apache-2.0">
<img alt="License" src="https://img.shields.io/badge/License-Apache_2.0-blue.svg" />
</a>
<a href="https://github.com/Evoland-Land-Monitoring-Evolution/sentinel2_superresolution/tags">
<img alt="Release Version" src="https://img.shields.io/github/v/tag/Evoland-Land-Monitoring-Evolution/sentinel2_superresolution" />
</a>
#+end_html
...
- [[#changelog][Changelog]]
  ...
  - [[#with-respect-to-version-201][With respect to version 2.0.1]]
...
The sentinel2_superesolution app can also be packaged as a docker image as such:

#+begin_src shell
docker build -f Dockerfile -t sentinel2_superresolution:latest ./ 
#+end_src

or directly pulled from the dockerhub:

#+begin_src shell
docker pull cnes/sentinel2_superresolution:X.Y.Z
#+end_src

The folder containing input products must be binded to the container using th ~-v~ flag, and the optional ~--gpus~ flag can be used to expose the GPUs to the container.

#+begin_src shell
docker run --rm --gpus all -v /media/data/sandbox/s2sr:/data sentinel2_superresolution:latest \
  python /app/run.py -i /data/S2A_MSIL1C_20250317T104811_N0511_R051_T31TCJ_20250317T132621.SAFE \
  --l1c --bicubic --region_of_interest_pixel 7200 6800 7700 7200 \
  --model /app/src/sentinel2_superresolution/models/carn_3x3x64g4sw_bootstrap.yaml \
  -o /data/results
#+end_src

** CWL Workflow

An example CWL Workflow can be found in the ~cwl/~ folder alogside with a sample ~input.yml~ file.
It can be executed an any cwl engine, for instance using ~cwltool~ locally, with and ~s3.env~ containing the credentials for the S3-like storage:

#+begin_src shell
source s3.env
cwltool --preserve-environment AWS_ACCESS_KEY_ID --preserve-environment AWS_SECRET_ACCESS_KEY s2-superresolution.cwl inputs.yml
#+end_src
